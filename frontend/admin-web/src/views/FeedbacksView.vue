...
<template>
  <div class="admin-page feedbacks-page">
    <div class="tb-header"><div><h2>反馈管理</h2><p class="page-subtitle">处理用户意见反馈，形成用户提交、后台回复、用户查看的闭环。</p></div></div>
    <div class="admin-summary">
      <div class="summary-card"><div class="summary-card__label">当前页反馈</div><div class="summary-card__value">{{ items.length }}</div><div class="summary-card__hint">分页记录</div></div>
      <div class="summary-card"><div class="summary-card__label">已回复</div><div class="summary-card__value">{{ repliedCount }}</div><div class="summary-card__hint">已完成闭环</div></div>
      <div class="summary-card"><div class="summary-card__label">待处理</div><div class="summary-card__value">{{ pendingCount }}</div><div class="summary-card__hint">需要后台响应</div></div>
      <div class="summary-card"><div class="summary-card__label">总记录</div><div class="summary-card__value">{{ total }}</div><div class="summary-card__hint">接口分页统计</div></div>
    </div>
    ...
    <el-table :data="items" stripe>
      ...
      <el-table-column label="操作" width="180" fixed="right">
        <template #default="{row}">
          <div class="action-stack"><el-button size="small" @click="openReply(row)">{{row.reply?'修改回复':'回复'}}</el-button>

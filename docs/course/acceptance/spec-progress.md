...
| 项目 | 建议动作 |
| --- | --- |
| 促销/秒杀库存闭环 | 用浏览器从首页促销商品进入详情、下单，再用后台或数据库确认库存变化 |
| 评价带图闭环 | 用已完成订单提交带图评价，检查商品详情和后台评价管理 |
| 图片上传入口 | 至少验证后台商品图、轮播图、评价图中的两类 |
| 首页热门搜索词/二级分类 | 检查首页首屏入口是否足够明显，必要时再补 UI |
| 前台响应式 | 390px 和 768px 巡检首页、商品详情、购物车、结算、订单 |
| 客服/公告/反馈闭环 | 按 `SPEC.md` 路线各走一遍，确认用户端和后台状态互通 |
| Legacy 证据 | 打开 `http://localhost:18080/legacy/status`，确认 Servlet/JSP/Listener/Filter/JDBC 信息可见 |

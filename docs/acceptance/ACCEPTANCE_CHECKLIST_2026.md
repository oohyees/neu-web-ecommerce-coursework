...
| 拓展 | 微服务 | ✅ | `services/gateway`, `services/*-service`, `libs/common`, `apps/web` | gateway(18090), auth-service(18091), catalog-service(18092), order-service(18093), admin-service(18094) | 多工程独立构建、网关转发 |
| 拓展 | Docker | ✅ | `apps/` + 外层微服务工程 | `docker-compose.yml`, `docker-compose.microservices.yml` | 主栈(单体+前端+MySQL+Redis)、微服务栈(网关+多服务+共享MySQL/Redis) |

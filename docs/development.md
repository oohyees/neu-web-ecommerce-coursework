...
| 商品图片 | 商品种子使用 DummyJSON CDN 远程图片 URL（`cdn.dummyjson.com`），194 条真实商品含品牌、评分、折扣、SKU 变体、图集和规格；商品详情页支持主图和图集（`product_image` 表） | 首次启动 Docker 数据库时自动初始化种子数据；后续修改需手动执行 SQL 或重建容器 |

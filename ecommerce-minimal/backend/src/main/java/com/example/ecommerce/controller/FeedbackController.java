...
public class FeedbackController {
    ...
    @GetMapping("/api/feedback") public ApiResponse<?> mine(@RequestParam(required=false) Long userId, HttpServletRequest request){return ApiResponse.ok(mapper.findByUserId(CurrentSession.userId(request)));}
    @PostMapping("/api/feedback") public ApiResponse<?> create(@RequestBody Feedback f, HttpServletRequest request){f.setUserId(CurrentSession.userId(request));f.setStatus("PENDING");f.setCreatedAt(LocalDateTime.now());mapper.insert(f);return ApiResponse.ok(f);}
